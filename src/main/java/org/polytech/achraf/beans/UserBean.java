...
private ArticleDao articleDao;
	...
	public UserBean() { 
		...
		transactionEntreeList = TransactionEntreeDao.getInstance().findAllMyTransactions();
	}
	...
	public void ajouterTransactionSortie() {
		...
		if(article == null)
			...
			if(article.getStock() - addedTransaction.getQuantite() < 0 )
				...
				if(transactionSortiList == null) 
					transactionSortiList = new ArrayList<TransactionSortie>();
	...
	public void updateTransactionSortie() {
		...
		if(article == null)
			...
			if(article.getStock() - selectedTransaction.getQuantite() < 0 )
				...
				PrimeFaces.current().executeScript("PF('manageProductDialog').hide();");
				transactionSortiList = TransactionSortieDao.getInstance().findAllMyTransactions();
	...
	public List<TransactionSortie> getTransactionSortiList() {
		return TransactionSortieDao.getInstance().findAllMyTransactions();
	}
	...
	public List<TransactionEntree> getTransactionEntreeList() {
		return TransactionEntreeDao.getInstance().findAllMyTransactions();
	}
